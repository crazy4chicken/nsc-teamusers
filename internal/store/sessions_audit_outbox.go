...
func CreateSession(ctx context.Context, q Q, session Session) (Session, error) {
	...
	return scanSession(q.QueryRow(ctx, `
		INSERT INTO sessions (id, user_id, family_id, client_meta, expires_at, family_not_after, revoked_at, revoke_reason)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
		RETURNING id, user_id, family_id, client_meta, expires_at, family_not_after, revoked_at, revoke_reason`,
		session.ID, session.UserID, session.FamilyID, clientMeta, session.ExpiresAt,
...
func scanSession(row pgx.Row) (Session, error) {
	...
	if err := row.Scan(
		...
		&session.ExpiresAt, &session.FamilyNotAfter, &session.RevokedAt, &reason,
	); err != nil {
		return Session{}, err
	}
