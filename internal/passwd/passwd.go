...
// NewDummyHash returns a valid Argon2id hash for a password that is never
// accepted by the application, for timing-equalized credential lookups.
func NewDummyHash() string {
	salt := []byte("teamusers-dummy0")
...
// NeedsRehash reports whether encoded is invalid or does not use the current
